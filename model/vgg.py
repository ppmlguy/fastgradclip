...
from fastgc.util import clip_tensor
...
def make_layers(cfg, conv_layers, batch_norm=False):
    layers = []
    in_channels = 3
...
def _vgg(arch, cfg, batch_norm, pretrained, progress, output_size, **kwargs):
    if pretrained:
        kwargs['init_weights'] = False
    model = VGG(output_size, cfg, batch_norm=batch_norm, **kwargs)
    if pretrained:
...
def vgg11(pretrained=False, progress=True, num_classes=10, **kwargs):
    ...
    """
    return _vgg('vgg11', 'A', False, pretrained, progress, num_classes, **kwargs)
...
def vgg13(pretrained=False, progress=True, num_classes=10, **kwargs):
